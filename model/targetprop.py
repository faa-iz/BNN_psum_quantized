...
def hinge(z, t, margin=1.0, trunc_thresh=float('inf'), scale=1.0):
    ...
    loss = ((-z * t.float() + margin) * scale).clamp(min=0, max=trunc_thresh)
    return loss
...
def soft_swish(z, t, beta=1.0, xscale=1.0, yscale=1.0):
    loss = beta * (2 - beta * -(z * t).float() * torch.tanh(beta * -(z * t).float() / 2))\
            / (1 + (torch.exp(beta * -(z * t).float()) + torch.exp(- beta * -(z * t).float()) / 2))
    return loss
...
class TPRule(Enum):
    ...
    @staticmethod
    def swish_backward(step_input, grad_output, target, is01, xscale=1.0, yscale=1.0):
        if target is None:
            target = torch.sign(-grad_output)
        beta = 15
        z = soft_swish(step_input, target, beta=beta, xscale=xscale, yscale=yscale)
        grad_input = z * xscale * yscale * -target / grad_output.size(0)
        return grad_input, None
    @staticmethod
    def bireal_backward(step_input, grad_output, target, is01, xscale=1.0, yscale=1.0):
        if target is None:
            target = torch.sign(-grad_output)
        grad_input = grad_output * (2 + 2 * step_input) * torch.le(step_input, 0).float() \
                    + grad_output * (2 - 2 * step_input) * torch.ge(step_input, 0).float()
        return grad_input, None
    @staticmethod
    ...
    def get_backward_func(targetprop_rule):
        ...
        elif targetprop_rule == TPRule.SWISH:
            tp_grad_func = TPRule.swish_backward
        elif targetprop_rule == TPRule.BIREAL:
            tp_grad_func = TPRule.bireal_backward
